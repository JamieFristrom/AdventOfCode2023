...
namespace pex {
	...
	[[nodiscard]] auto pReduce(const Container& c, UnaryOperation unaryOp) {
...
template <typename T>
static Vec3<T> operator*(const T k, const Vec3<T>& v) { return v * k; }

template<typename T>
Vec3<T> crossProduct(const Vec3<T>& v_A, const Vec3<T>& v_B) {
	Vec3<T> c_P;
...
bool approxEqual(const T a, const T b, const U epsilon=0.0001) {
	return a >= b - epsilon && a <= b + epsilon;
}

bool approxEqual(const Vec3<double>& v3a, const Vec3<double>& v3b, const double epsilon=0.001) {
...
template <typename T>
std::ostream& operator<<(std::ostream& os, const Vec3<T>& v3) {
	os << "[" << v3.x << " " << v3.y << " " << v3.z << "]";
	return os;
}
