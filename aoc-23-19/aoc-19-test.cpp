...
struct Part {
    std::map<char, int64_t> stats;

    const bool operator==(const Part& p) const { return p.stats == stats; }
};

using Parts = std::vector<Part>;

std::pair<Workflows, Parts> parseInput(const std::string& input) {
    Workflows workflows;
    Parts parts;
    const auto lines = pSplit(input, '\n');
    auto line_it = lines.begin();
    for (; !line_it->empty(); ++line_it ) {
        std::regex regExpTest("Wah");
        std::regex regExp("([a-z]+)\\{(.*)\\}");  // {} has meaning in C++ regex, needs \ to indicate literal, and c++ needs \ to indicate the \ is literal  :P
        std::smatch resultMatch;
        const bool parsed = regex_match(*line_it, resultMatch, regExp);
        assert(parsed);
        const auto workflowName = resultMatch[1];
        const auto workflowStr = resultMatch[2];
        const auto ruleStrs = pSplit(workflowStr, ',');
        ...
        for (const auto& ruleStr : ruleStrs) {
            if (ruleStr.empty()) {
                ...
                newWorkflow.emplace_back(Rule{ std::nullopt,ruleStr });
            }
            else {
                std::regex regExp("([a-z])([<>])([0-9]+):([a-zA-Z]+)");
                std::smatch resultMatch;
                const bool parsed = regex_match(ruleStr, resultMatch, regExp);
                assert(parsed);
                Condition condition{ .category = resultMatch[1].str()[0],
                    .op = (resultMatch[2] == ">") ? Condition::Op::GT : Condition::Op::LT,
                    .rating = stoi(resultMatch[3]) };
                Rule newRule{ condition, resultMatch[4] };
                newWorkflow.emplace_back(newRule);
            }
        }

        workflows[workflowName] = newWorkflow;
    }
    ++line_it; // onto the parts
    for (; line_it != lines.end(); ++line_it) {
        std::regex regExp("\\{([a-z])=([0-9]+),([a-z])=([0-9]+),([a-z])=([0-9]+),([a-z])=([0-9]+)\\}");
        std::smatch resultMatch;
        const bool parsed = regex_match(*line_it, resultMatch, regExp);
        assert(parsed);
        Part newPart;
        for (auto submatch = resultMatch.begin()+1; submatch != resultMatch.end(); ++submatch) {  //+1 because first submatch is always whole thing
            auto statId = submatch->str()[0];
            ++submatch;
            auto rating = stoi(submatch->str());
            newPart.stats[statId] = rating;
        }
...
bool shuntPart(const Workflows& workflows, const Part& part, const std::string& curWorkflow) {
    if (curWorkflow == "A") {
        return true;
    }
    if (curWorkflow == "R") {
    ...
    const auto workflow = workflows.at(curWorkflow);
    for (const Rule& rule : workflow) {
        if (const auto condition = rule.condition) {
            if (condition->op == Condition::Op::GT) {
                if (part.stats.at(condition->category) > condition->rating) {
                    return shuntPart(workflows, part, rule.destination);
                }
            }
            else {
                if (part.stats.at(condition->category) < condition->rating) {
    ...
    assert(false); // last step in workflow is supposed to be a catch-all
    return false;
}

using PartSpan = std::pair<Part, Part>;

std::vector<PartSpan> divideParts(const Workflows& workflows, const PartSpan& partSpan, const std::string& curWorkflow) {
    if (curWorkflow == "A") {
        // accept it all!
        return { partSpan };
    }
    ...
    std::vector<PartSpan> divisions;
    const auto workflow = workflows.at(curWorkflow);
    PartSpan remainingParts = partSpan;
    for (const Rule& rule : workflow) {
        if (const auto condition = rule.condition) {
            if (condition->op == Condition::Op::GT) {
                if (remainingParts.second.stats[condition->category] > condition->rating) {
                    // at last some parts in the space need to be funneled into the new workflow
                    PartSpan partSplit = remainingParts;
                    // some or all get shunted
                    partSplit.first.stats[condition->category] = std::max(  
                        partSplit.first.stats[condition->category],
                        condition->rating + 1);
                    auto spans = divideParts(workflows, partSplit, rule.destination);
                    divisions = pConcatenate(divisions, spans);
                    // any left?
                    remainingParts.second.stats[condition->category] = std::min(
                        remainingParts.second.stats[condition->category],
                        condition->rating
                    );
                    if (remainingParts.first.stats[condition->category] >
                        remainingParts.second.stats[condition->category]) {
                ...
                if (remainingParts.first.stats[condition->category] < condition->rating) {
                    // at last some parts in the space need to be funneled into the new workflow
                    PartSpan partSplit = remainingParts;
                    // some or all get shunted
                    partSplit.second.stats[condition->category] = std::min(
                        partSplit.second.stats[condition->category],
                        condition->rating - 1);
                    auto spans = divideParts(workflows, partSplit, rule.destination);
                    divisions = pConcatenate(divisions, spans);
                    // any left?
                    remainingParts.first.stats[condition->category] = std::max(
                        remainingParts.first.stats[condition->category],
                        condition->rating
                    );
                    if (remainingParts.first.stats[condition->category] >
                        remainingParts.second.stats[condition->category]) {
            ...
            auto spans = divideParts(workflows, remainingParts, rule.destination);
            divisions = pConcatenate(divisions, spans);
        }
    }
    return divisions;
...
int64_t partValue(const Part& part) {
    return pReduce(pTransform<std::vector<int64_t>>(part.stats, [](const auto& pair) { return pair.second; }));
}

int64_t doTheThing(const std::string& input) {
    const auto [workflows, parts] = parseInput(input);
    const auto values = pTransform<std::vector<int64_t>>(parts, [&workflows](const auto& part) { 
        return shuntPart(workflows, part, "in") ? partValue(part) : 0ll;
        });
    return pReduce(values);
}

int64_t doTheThing2(const std::string& input) {
    const auto [workflows, parts] = parseInput(input);
    const PartSpan partSpace = {
        Part{.stats = {{'x',1},{'m',1},{'a',1},{'s',1}}},
        Part{.stats = {{'x',4000},{'m',4000},{'a',4000},{'s',4000}}}
    };
    const auto partSpans = divideParts(workflows, partSpace, "in");
    const auto values = pTransform<std::vector<int64_t>>(partSpans, [](const auto& partSpan) {
        return (partSpan.second.stats.at('x') - partSpan.first.stats.at('x') + 1) *
            (partSpan.second.stats.at('m') - partSpan.first.stats.at('m') + 1) *
            (partSpan.second.stats.at('a') - partSpan.first.stats.at('a') + 1) *
            (partSpan.second.stats.at('s') - partSpan.first.stats.at('s') + 1);
        });
    return pReduce(values);
}
...
TEST(Aoc19Test, sampleInput_parse) {
    auto [workflows, parts] = parseInput(sampleInput);

    ASSERT_EQ(11, ssize(workflows));
    ASSERT_EQ(5, ssize(parts));

    Part expectedPart{ .stats = {{'x',2127},{'m',1623},{'a',2188},{'s',1013}} };
    ASSERT_EQ(expectedPart, parts[4]);

    ASSERT_EQ(true, shuntPart(workflows, parts[0], "in"));
    ASSERT_EQ(false, shuntPart(workflows, parts[1], "in"));
    ASSERT_EQ(true, shuntPart(workflows, parts[2], "in"));
    ASSERT_EQ(false, shuntPart(workflows, parts[3], "in"));
    ASSERT_EQ(true, shuntPart(workflows, parts[4], "in"));
}

TEST(Aoc19Test, eachStat1_partValue_4) {
    ASSERT_EQ(4, partValue(Part{ .stats = {{'x',1}, {'m',1}, {'a',1}, {'s',1 }} }));
}
TEST(Aoc19Test, sampleInput_doTheThing_19114) {
    ASSERT_EQ(19114, doTheThing(sampleInput));
}

TEST(Aoc19Test, sampleInput_doTheThing2_167409079868000) {
    ASSERT_EQ(167409079868000, doTheThing2(sampleInput));
}

const std::string puzzleInput = 
R"(vm{a>1183:zz,s>1553:A,s<827:xc,hls}
...
TEST(Aoc19Test, puzzleInput_doTheThing) {
    ASSERT_EQ(0, doTheThing(puzzleInput));
}

TEST(Aoc19Test, puzzleInput_doTheThing2) {
